        ...
        >
            <div className="flex items-center justify-center gap-4">
                ...
                <motion.button
                    ...
                    aria-label={isPaused ? 'Devam et' : 'Solo başla'}
                    className={`flex items-center gap-2 text-lg font-bold px-12 py-4 rounded-2xl transition-all ${canStart && !isRestricted
                        ? 'bg-[#eea62b] hover:bg-[#d4941f] text-[#221b10] shadow-[0_20px_60px_rgba(238,166,43,0.35)]'
                ...
                {!isRestricted && !isPaused && (
                    <motion.button
                        ...
                        disabled={!canStart}
                        aria-label="Eşleşme bul"
                        className={`flex items-center gap-2 text-lg font-bold px-8 py-4 rounded-2xl transition-all ${canStart
                            ? 'bg-white/[0.06] border border-[#eea62b]/30 text-white hover:bg-[#eea62b]/10 shadow-xl'
            ...
            {/* FOMO: Kuyruk teaser */}
            {!isPaused && waitingCount > 0 && (
                <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="flex items-center gap-1.5 text-[#eea62b]/70 text-xs font-medium"
                >
                    <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" />
                    {waitingCount} kişi şu an eşleşme bekliyor
                </motion.p>
            )}
        </motion.div>
    );
